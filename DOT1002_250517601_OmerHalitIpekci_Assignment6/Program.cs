...
namespace DOT1002_250517601_OmerHalitIpekci_Assignment6
{
    ...
    internal class EndingManager
    {
        public bool CheckFinalInput(string input)
        {
            ...
            else
            {
                ...
                if (loweredInput.Contains("look"))
                {
                    safe = false;
                }

                if (loweredInput.Contains("back"))
                {
                    safe = false;
                }

                if (loweredInput.Contains("turn"))
                {
                    safe = false;
                }

                if (loweredInput.Contains("check"))
                {
                    safe = false;
                }
            }
    ...
    internal class Inventory
    {
        ...
        {
            bool alreadyExists = HasItem(item.ItemName);
    ...
    {
        public CollectibleItem(string itemName, string description)
    ...
    internal abstract class Enemy : Character
    {
        ...
        public Enemy(string name, int maxHp, int currentHp, int damageAmount)
            : base()
        {
            this.name = name;
            this.maxHp = maxHp;
    ...
    internal class Cerberus : Enemy, IInteractable
    {
        private bool isCalmed;

        public Cerberus()
            : base("Cerberus", 20, 20, 5)
        {
            isCalmed = false;
        }
    ...
    internal class SkeletonEnemy : Enemy
    {
        public SkeletonEnemy()
            : base("Skeleton", 10, 10, 5)
        {
        }
    }

    internal class Eagle : Enemy
    {
        public Eagle()
            : base("Eagle", 8, 8, 4)
        {
    ...
    internal class Hades : NPC
    {
        private int favorRequired;

        public int FavorRequired
        {
            get { return favorRequired; }
            set { favorRequired = value; }
        }
    ...
    internal class CaveEntranceLocation : Location
    {
        public CaveEntranceLocation()
            : base("Cave Entrance")
        {
            enemies.Add(new Cerberus());
            items.Add(new CollectibleItem("Spirit Charm", "Increases Wisdom."));
            items.Add(new CollectibleItem("Memory Fragment", "Increases Intelligence."));
        }
    }

    internal class RiverOfEchoesLocation : Location
    {
        public RiverOfEchoesLocation()
            : base("River of Echoes")
        {
            npcs.Add(new Charon());
            npcs.Add(new Odysseus());
            enemies.Add(new SkeletonEnemy());
        }
    }

    internal class ForgottenCatacombsLocation : Location
    {
        public ForgottenCatacombsLocation()
            : base("Forgotten Catacombs")
        {
            npcs.Add(new Prometheus());
            enemies.Add(new Eagle());
            items.Add(new CollectibleItem("Titan Bone", "Increases Strength."));
            items.Add(new CollectibleItem("Vital Core", "Increases Constitution and HP."));
            items.Add(new CollectibleItem("Song of Silence", "Increases Charisma."));
        }
    }

    internal class AshenFieldsLocation : Location
    {
        public AshenFieldsLocation()
            : base("Ashen Fields")
        {
            npcs.Add(new Zagreus());
            enemies.Add(new SkeletonEnemy());
            puzzles.Add(new AshTrialPuzzle());
        }
    }

    internal class HallOfBrokenOathsLocation : Location
    {
        public HallOfBrokenOathsLocation()
            : base("Hall of Broken Oaths")
        {
            npcs.Add(new Cronos());
            npcs.Add(new Thanatos());
            puzzles.Add(new GreatDoorPuzzle());
        }
    }

    internal class HouseOfHadesLocation : Location
    {
        public HouseOfHadesLocation()
            : base("House of Hades")
        {
            npcs.Add(new Hypnos());
            npcs.Add(new Hades());
    ...
    internal abstract class Encounter : ICheckable
    {
        ...
        public Encounter(string encounterName, int difficultyClass)
        {
        ...
        public virtual bool PerformCheck(Player player)
        {
            return false;
        }
    ...
    internal class Puzzle
    {
        ...
        public virtual bool Attempt(Player player)
        {
            return false;
        }
    }

    internal class GreatDoorPuzzle : Puzzle, IInteractable
    {
        private List<string> requiredWords;

        public GreatDoorPuzzle()
            : base("Great Door Puzzle")
        {
            requiredWords = new List<string>();
            requiredWords.Add("Burden");
            requiredWords.Add("Love");
            requiredWords.Add("Loss");
        }
        ...
        public bool CheckWords(string firstWord, string secondWord, string thirdWord)
        {
            bool correct = false;
    ...
    internal class AshTrialPuzzle : Puzzle, ICheckable
    {
        private List<string> correctPattern;
        private int currentStep;

        public AshTrialPuzzle()
            : base("Ashen Fields Trial")
        {
            correctPattern = new List<string>();
            correctPattern.Add("forward");
            correctPattern.Add("forward");
            correctPattern.Add("left");
            correctPattern.Add("right");
            correctPattern.Add("left");
            correctPattern.Add("right");
            correctPattern.Add("forward");
            correctPattern.Add("forward");
        ...
        public bool CheckMovement(string movement)
        {
            bool correct = false;
    ...
    internal class GameManager
    {
        ...
        // Stats are kept in Player, but old game code uses these names.
        private int strength
        {
            get { return player.Strength; }
            set { player.Strength = value; }
        }

        private int dexterity
        {
            get { return player.Dexterity; }
            set { player.Dexterity = value; }
        }

        private int constitution
        {
            get { return player.Constitution; }
            set { player.Constitution = value; }
        }

        private int intelligence
        {
            get { return player.Intelligence; }
            set { player.Intelligence = value; }
        }

        private int wisdom
        {
            get { return player.Wisdom; }
            set { player.Wisdom = value; }
        }

        private int charisma
        {
            get { return player.Charisma; }
            set { player.Charisma = value; }
        }
        ...
        private string nextArea = "cave";
        private string boatType = "";
        private bool enteredAshByBoat = false;

        // Collectible checks are connected to the player's inventory.
        private bool hasSongOfSilence
        {
            get { return player.Inventory.HasItem("Song of Silence"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Song of Silence", "Increases Charisma.")); } }
        }

        private bool hasTitanBone
        {
            get { return player.Inventory.HasItem("Titan Bone"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Titan Bone", "Increases Strength.")); } }
        }

        private bool hasShadowCloak
        {
            get { return player.Inventory.HasItem("Shadow Cloak"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Shadow Cloak", "Increases Dexterity.")); } }
        }

        private bool hasMemoryFragment
        {
            get { return player.Inventory.HasItem("Memory Fragment"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Memory Fragment", "Increases Intelligence.")); } }
        }

        private bool hasSpiritCharm
        {
            get { return player.Inventory.HasItem("Spirit Charm"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Spirit Charm", "Increases Wisdom.")); } }
        }

        private bool hasVitalCore
        {
            get { return player.Inventory.HasItem("Vital Core"); }
            set { if (value == true) { player.Inventory.AddItem(new CollectibleItem("Vital Core", "Increases Constitution and HP.")); } }
        }
        ...
        public GameManager()
        {
            ...
            locations.Add(new CaveEntranceLocation());
            locations.Add(new RiverOfEchoesLocation());
            locations.Add(new ForgottenCatacombsLocation());
            locations.Add(new AshenFieldsLocation());
            locations.Add(new HallOfBrokenOathsLocation());
            locations.Add(new HouseOfHadesLocation());
        ...
        // Cave flags
        bool didCerberus = false;
        bool cerberusCalmed = false;
        bool didExamineCave = false;


        // Catacombs flags
        bool didPrometheus = false;
        bool didPillar = false;
        bool freedPillarSoul = false;
        bool didWhispers = false;
        bool failedWhispersOnce = false;
        bool helpedPrometheus = false;
        bool failedPrometheusOnce = false;
        bool didChaosGate = false;
        bool foundHiddenPath = false;

        // Ash flags
        bool didAshBattlefield = false;
        bool ashBattlefieldCalmed = false;
        bool ashBattlefieldEscaped = false;
        bool didAshSearch = false;
        bool didZagreus = false;
        bool knowsAshPattern = false;
        bool crossedAsh = false;

        // Oath flags
        bool gotBurdenWord = false;
        bool gotLoveWord = false;
        bool gotLossWord = false;

        bool didCronos = false;
        bool didGarden = false;
        bool didThanatos = false;

        bool knowsCronos = false;
        bool knowsGardenTruth = false;
        bool knowsThanatos = false;

        // Hades flags
        bool metHypnos = false;
        int hadesFavor = 0;
        // Hades bonus usage flags
        bool usedBonusCerberus = false;
        bool usedBonusCharon = false;
        bool usedBonusOdysseus = false;
        bool usedBonusPrometheus = false;
        bool usedBonusThanatos = false;
        bool usedBonusGarden = false;
        bool usedBonusCronos = false;

        // Lore flags
        bool knowsPrometheus = false;
        ...
        void WriteSlow(string text, int delayMs = 15)
        {
        ...
        void WriteLineSlow(string text, int delayMs = 15)
        {
            WriteSlow(text, delayMs);
            Console.WriteLine();
        }

        bool VisibleCheck(string checkName, int statValue, int dc)
        {
            int roll = RollD20();
        ...
        bool HiddenCheck(int statValue, int dc)
        {
            int roll = RollD20();
            int modifier = GetModifier(statValue);
            int total = roll + modifier;
        ...
        void AddCollectible(string collectibleName)
        {
            Console.WriteLine();
        ...
        void ExamineCave()
        {
            ...
            {
                WriteLineSlow("You search the cave again.");

                if (hasMemoryFragment == true)
                {
                    WriteLineSlow("The strange light has already given you what it could.");
                }
                else
                {
                    WriteLineSlow("The darkness hides its secrets from you now.");
                }
            ...
            bool examineSuccess = HiddenCheck(intelligence, 8);
            didExamineCave = true;
        ...
        bool triedCharonTalk = false;
        bool triedCharonLyre = false;
        bool usedOdysseus = false;
        ...
        void PrometheusEncounter()
        {
            ...
            if (failedPrometheusOnce == true)
            {
                WriteLineSlow("It is the same eagle from before.");
                WriteLineSlow("It was not killed or defeated last time.");
                WriteLineSlow("It only kept its distance for a while, and now it circles again.");
            }
            ...
            else
            {
                ...
                if (gameEnded == false)
                {
                    WriteLineSlow("The eagle is not dead.");
                    WriteLineSlow("It only pulls away for a short while, circling above the mountain again.");
                    WriteLineSlow("Prometheus remains chained, but you may still try to help him if you return.");
                    Console.WriteLine();
                }
            }
        ...
        void PillarEncounter()
        {
            if (didPillar == true)
            {
                ...
                {
                    WriteLineSlow("The stone still lies where it fell.");
                    WriteLineSlow("The faint voice is gone now.");
                    WriteLineSlow("You were not able to save the soul beneath it.");
                }
            ...
            else
            {
                ...
                WriteLineSlow("By the time you step back, the voice has faded away.");
                Console.WriteLine();
            }
        ...
        bool WhispersEncounter()
        {
            ...
            if (failedWhispersOnce == true)
            {
                WriteLineSlow("You listen again.");
                WriteLineSlow("The voices still remember your last failed attempt.");
                WriteLineSlow("They are restless, but they have not fully disappeared.");
                Console.WriteLine();
            }
            ...
            {
                failedWhispersOnce = true;

                WriteLineSlow("The voices grow louder.");
                WriteLineSlow("You cannot separate them.");
                WriteLineSlow("You step back.");
                WriteLineSlow("But they remain, unsettled by your failed attempt.");
                WriteLineSlow("You can try again, but the voices will remember this.");
                Console.WriteLine();
                return false;
            }
            ...
            WriteLineSlow("The voices do not forget the broken song, but they still linger in the dark.");
            Console.WriteLine();
            return false;
        }

        void ChaosGateEncounter()
        {
            ...
            WriteLineSlow("Something in the air feels different.");
        ...
        bool gotBoatCollectible = false;
        bool talkedToOdysseus = false;
        ...
        void NextAfterRiver()
        {
            ...
            if (boatType == "charon")
            {
                ...
                Console.WriteLine();
                WriteLineSlow("Where do you ask Charon to stop?");
                WriteLineSlow("1. Stop near the ruins");
                WriteLineSlow("2. Stop near the ash-covered fields");
                ...
                {
                    WriteLineSlow("Charon guides the boat toward the ruined dock.");
                    nextArea = "catacombs";
                    Pause();
                }
                ...
                {
                    WriteLineSlow("Charon guides the boat toward the burning horizon.");
        ...
        void AshBattlefieldEncounter()
        {
            if (didAshBattlefield == true)
            {
                WriteLineSlow("You return to the ruined battlefield.");

                if (ashBattlefieldCalmed == true)
                {
                    WriteLineSlow("The skeletons remain collapsed beneath the ash.");
                    WriteLineSlow("Your song has already quieted this place.");
                }
                else if (ashBattlefieldEscaped == true)
                {
                    WriteLineSlow("The ash shifts where the skeletons chased you before.");
                    WriteLineSlow("They are not destroyed, but they do not rise again while you keep your distance.");
                    WriteLineSlow("You already know this battlefield is dangerous.");
                }
                else
                {
                    WriteLineSlow("The battlefield is quiet now.");
                }
            ...
            if (choice == 3)
            {
                ...
                ashBattlefieldEscaped = true;
                ashBattlefieldCalmed = false;
            ...
            if (choice == 2)
            {
                ...
                {
                    didAshBattlefield = true;
                    ashBattlefieldCalmed = true;
                    ashBattlefieldEscaped = false;
        ...
        void AshSearchEncounter()
        {
            if (didAshSearch == true)
            {
                ...
                {
                    WriteLineSlow("You already understand the rhythm beneath the ash.");
                }
                else
                {
                    WriteLineSlow("The ash shifts under your hands, but the pattern still escapes you.");
                }
        ...
        void ZagreusEncounter()
        {
            ...
            WriteLineSlow("\"Forward. Forward. Left. Right. Left. Right. Forward. Forward.\"");
            Console.WriteLine();
        ...
        void HadesThroneRoom()
        {
            ...
            if (gameEnded == false)
            {
                ...
                {
                    WriteLineSlow("Bad Ending: Hades refuses your request.");
                    Console.WriteLine();
        ...
        void EscapeSequence()
        {
            ...
            if (gameEnded == false)
            {
                ...
                bool finalSave = HiddenCheck(charisma, 10);
                ...
                else
                {
                    ...
                    bool safeInput = endingManager.CheckFinalInput(finalInput);
        ...
        void EscapeStep(string description, string forwardText, string backText)
        {
            Console.Clear();
